...
def render_plant_page(plant, prev_p, next_p):
  ...
  inline_image = (
    ...
    if inline_src else
    f'<img src="{esc(inline_external)}" alt="{esc(plant["display_name"])} reference detail" loading="lazy">'
    if inline_external else ""
  )
  inline_photo = (
...
.index-link { color: var(--blue); font-size: .96rem; padding: .32rem 0; }
...
.plant-hero-image { min-height: 0; height: 405px; margin: 1rem 1rem 1rem 0; padding: 5px; background: rgba(244,239,228,.88); position: relative; border: 8px solid rgba(244,239,228,.88); border-radius: 9px; overflow: hidden; transform: rotate(1.5deg); box-shadow: 0 18px 30px rgba(13,34,22,.28); z-index: 1; }
